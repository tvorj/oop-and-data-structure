...
namespace container
{
    ...
    private:
        ...
        { // Bug->Last element not removed/crash
    ...
    public:
        ...
        {
            return remove_if([&toRemove_value](auto other)
                             { return other == toRemove_value; });
        }
        ...
        constexpr size_type remove_if(Predicate pred)
        {
            ...
            {
                if (pred(first->data))
                { // Lambda is true, remove the element - m_head might be changed
                    if (first == m_head)
                    {
